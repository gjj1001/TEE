...
import com.tee686.utils.ObserverPopupWindow;
...
//				mAdapter.appendToList(result);		
				...
				lv.setOnItemClickListener(new OnItemClickListener() {
					...
					public void onItemClick(AdapterView<?> parent, View view,
						...
						IntentUtil.startActivity(UserMessageActivity.this, UserInfoActivity.class,
								new BasicNameValuePair("userhead", observer.getHeadimage()),
								new BasicNameValuePair("username", observer.getUsername()),
								new BasicNameValuePair("position", String.valueOf(position)));
					}
				});
				//自己才能删除
				if(share.getString(UserLoginActivity.UID, "").equals(intent.getStringExtra("uname"))) {
					lv.setOnItemLongClickListener(new OnItemLongClickListener() {
						...
						public boolean onItemLongClick(AdapterView<?> parent,
							...
							tabs.add("删除");
							ObserverPopupWindow<String> util = new ObserverPopupWindow<String>(UserMessageActivity.this, 
									intent.getStringExtra("uname"), observer, share, mAdapter, mAdapter.observers);
...
//				result.clear();
			...
			url1 = String.format(Urls.USER_OBSERVER+"?uname=%s&ifmsg=%s&flag=%s", intent.getStringExtra("uname"), "true","true");
	...
	class DataAsyncTask extends AsyncTask<String, Void, List<Observer>> {
		...
		protected List<Observer> doInBackground(String... params) {			
			...
			try {				
				result = HttpUtils.getByHttpClient(UserMessageActivity.this, params[0]);					
...
//				mAdapter.appendToList(result);		
				...
				lv.setOnItemClickListener(new OnItemClickListener() {
					...
					public void onItemClick(AdapterView<?> parent, View view,
						...
						// TODO Auto-generated method stub
						Observer observer = (Observer) mAdapter.getItem(position);
						IntentUtil.startActivity(UserMessageActivity.this, ReplyActivity.class,
								new BasicNameValuePair("uname", observer.getUname().equals(
										share.getString(UserLoginActivity.UID, ""))?observer.getUsername():observer.getUname()));
					}
				});
				//自己才能删除
				/*if(share.getString(UserLoginActivity.UID, "").equals(intent.getStringExtra("uname"))) {
...
//				result.clear();
	...
	class MyAdapter extends BaseAdapter {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			 ...
			 */
			viewHolder.tvUsername.setText(observer.getUsername().equals(
					share.getString(UserLoginActivity.UID, ""))?observer.getUname():observer.getUsername());
			// viewHolder.ivHeadimage.setTag(pubContent.getHeadimage());
			// viewHolder.ivImagefile.setTag(pubContent.getImageFile());
			String name = observer.getUsername().equals(
					share.getString(UserLoginActivity.UID, ""))?observer.getUname():observer.getUsername();
			synchronized (viewHolder) {
				if(name.equals(observer.getUsername())) {
					if (!"".equals(observer.getHeadimage())) {
						// new
						// HeadImgAsyncTask().execute(pubContent.getHeadimage());
						ImageUtil.setThumbnailView(observer.getHeadimage(),
								viewHolder.ivHeadimage, UserMessageActivity.this,
								callback1, true);
					}
				} else {
